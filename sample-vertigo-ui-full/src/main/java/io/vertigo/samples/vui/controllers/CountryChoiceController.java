...
public final class CountryChoiceController extends AbstractVSpringMvcController {
	...
	public void initContext(final ViewContext viewContext, final SampleUserSession session) {
		viewContext.publishDto(userKey, obtainUser(session));
		viewContext.publishMdl(countriesKey, Country.class, null);
	}

	@PostMapping("/_changeCountry")
	public ViewContext doChange(final ViewContext viewContext, @ViewAttribute("user") final User user, final SampleUserSession session) {
		final User sessionUser = obtainUser(session);
