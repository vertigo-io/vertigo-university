...
jQuery(document).ready(function () {
	jQuery.ajaxSetup({type: "POST"}); //On post les requetes ajax pour pouvoir récupérer le context
...
function getResponseId(content) {
	...
	baliseIdentifiant = new String(content.match(regexBaliseIdenditiant))
			.split(",")[0];
	// on récupère l'identifiant
	identifiant = null;
	if (baliseIdentifiant != null) {
		identifiant = new String(baliseIdentifiant
				.match(regexIdentifiantBalise));
...
function getResponseContent(content) {
	...
	typeBalise = new String(baliseIdentifiant.match(regexTypeBalise))
			.split(",")[0];

	// on retire les balise identifiant le contenu ajax à remplacer
	var ret = content.substring(baliseIdentifiant.length, content.length
			- typeBalise.length - 3);
...
function putInWait(element, srcImageWait) {
	element.innerHTML = "<div style='margin-top:50px;text-align:center;'><img src='"
			+ srcImageWait + "'></div>";
}
